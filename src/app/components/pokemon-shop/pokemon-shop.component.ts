...
import {
  LocalStorageKey,
  LocalStorageService,
} from '../shared/services/localStorage/local-storage.service';
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
      <div style="margin-left: 5px;">
        <a class="nav-link disabled"
          >My Pokemon: {{ pokemonFacade.getPokemonTotal() | async }}ea</a
        >
      </div>
    </nav>
...
export class PokemonShopComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let forceLoginSub = this.authFacdes.isForceLogin().subscribe({
      next: (value) => {
        if (value) {
